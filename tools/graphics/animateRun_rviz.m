function [ ] = animateRun_rviz( logger, scale_factor )
...
for i = 1:length(logger)
    ...
    if hasFlow
        calcs(i) = logger(i).flow;
        % t  = logger(i).flow.t;
        % qb = logger(i).flow.states.x(1:6,   :);
        % q  = logger(i).flow.states.x(7:end, :);
        hz = 200;
        
        run('Hacks/recover_BasePosX_patch.m'); %tmp hack, need to fix this.
        
        [~,qb]  = even_sample(calcs(i).t, calcs(i).states.x(1:6, :), hz);
        [t,q]   = even_sample(calcs(i).t, calcs(i).states.x(7:end, :), hz);
