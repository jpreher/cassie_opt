function nlp = loadEdgeInitialGuess(nlp, src, tar)
    ...
    cstr_names = fieldnames(plant.ImpactConstraints);
    if ~isempty(cstr_names)
        [~, ~, lambda] = calcDiscreteMap(plant, src_calcs.t(end),[src_calcs.states.x(:,end);src_calcs.states.dx(:,end)]);
