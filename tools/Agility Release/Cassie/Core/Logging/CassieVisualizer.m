...
%       obj.tree.getEndEffectorByName('right-plantar-rod-end').isVisible = false;
      ...
      joints = obj.tree.getJoints;
      for i = 1:length(joints)
          joints{i}.isVisible = false;
      end
      
      bodies = obj.tree.getBodies;
      for i = 1:length(bodies)
          bodies{i}.isCenterOfMassVisible = false;
      end
      
      ee = obj.tree.getEndEffectors;
      for i = 1:length(ee)
          ee{i}.isVisible = false;
      end
    ...
    function timerFcn(obj, src, ~)
      ...
      % Give fixed figure limits
      xlim([-1,1]);
      ylim([-1,1]);
      zlim([-0.25, 1.25]);
      
      % Draw patch for ground
      v = [-2 -2; 2 -2; 2 2; -2 2];
      f = [1 2 3 4];
